...
4. **What happens during diagnosis** (brief, so they know what they're consenting to): a 7-step audit that is READ-ONLY on their codebase — it reads context, audits interfaces, inventories tests, outputs a gap list, and asks the user to fill critical parameters. No code changes in the diagnosis round.
...
>
> Diagnosis is read-only (reads your code, audits interfaces, inventories tests, outputs a gap list, asks you to fill acceptance criteria). I won't modify code or install anything without your explicit go-ahead.
>
...
2. **Execute the protocol exactly as written there.** That file declares its own self-boot protocol; follow it. Pay special attention to:
   ...
   - The diagnosis round is read-only on production code. Only AGENTS.md may be written. Code remediation waits for explicit confirmation in a later round.
